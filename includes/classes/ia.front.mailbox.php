...
class iaMailbox extends abstractCore
{
	...
	public function getFolders($userId)
	{
		$sql =
			'SELECT ' .
				'f.*, ' .
				'COUNT(m.`id`) `messages`, ' .
				'COUNT(1 = m.`new` OR NULL) `unread_messages`' .
	...
	{
		$this->iaDb->delete('`folder_id` = :folder AND `from_member_id` = :member', self::getTable(), array('folder' => self::TRASH, 'member' => $memberId));
	}
	...
	{
		return $this->iaDb->delete('`member_id` = :user AND `blocked_member_id` = :blocked_user', self::$_ignoreTable, array('user' => (int)$userId, 'blocked_user' => (int)$userIdToBeRemoved));
	}
